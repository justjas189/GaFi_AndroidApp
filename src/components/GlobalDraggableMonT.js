...
const GlobalDraggableMonT = () => {
  ...
  const handleBubbleTap = () => {
    ...
    mascot.triggerMascotReaction('CHAT_STARTED', {
      customMessage: "Hi! Let's chat about your finances! 💰"
    });
  };
  ...
  // Listen to mascot state changes
  useEffect(() => {
    if (mascot.showBubble && mascot.bubbleText) {
      showNotificationBubble(mascot.bubbleText);
    }
  }, [mascot.showBubble, mascot.bubbleText]);
    ...
    <>
      ...
      >
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          {/* MonT Mascot - Piggy Bank Image */}
          <MascotImage size={80} />
          
          {/* Notification Badge */}
          {mascot.notificationCount > 0 && (
            <View style={[styles.notificationBadge, { backgroundColor: '#FF4444' }]}>
              <Text style={styles.badgeText}>
                {mascot.notificationCount > 99 ? '99+' : mascot.notificationCount}
              </Text>
            </View>
          )}
...
export default GlobalDraggableMonT;
